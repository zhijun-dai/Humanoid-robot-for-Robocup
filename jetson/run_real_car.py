...
def main():
    ...
    while True:
        ...
        if lost == 0:
            ...
            derr = (err - last_err) / max(dt, 1e-3) / spd_f
            last_err = err
            ...
            if lost <= hold_frames:
                ...
                phase = (lost // 8) % 2
                grow = min(2.0, 1.0 + (lost - hold_frames) * 0.05)  # gradual growth over ~20 frames
                steer = (abs(SRCH_TURN) * spd_f ** 0.5 * grow) if phase == 0 else (-abs(SRCH_TURN) * spd_f ** 0.5 * grow)
            integral *= 0.5
        ...
        def _rad2byte(v):
            return max(-127, min(127, int(round(v * 10.0)))) & 0xFF
        payload = bytes([_rad2byte(fr), _rad2byte(fl),
                        _rad2byte(rr), _rad2byte(rl)])
        xor_chk = payload[0] ^ payload[1] ^ payload[2] ^ payload[3]
        frame = bytes([0xFF]) + payload + bytes([xor_chk, 0xEE])
        _serial_send(frame)

        # ── Console print (every PRINT_INTERVAL seconds) ──
        if t - last_print_t > PRINT_INTERVAL:
            last_print_t = t
            print(f"{frame[0]:02X}{frame[1]:02X}{frame[2]:02X}{frame[3]:02X}{frame[4]:02X}{frame[5]:02X}{frame[6]:02X}")
        ...
        if extra:
            _put(frame_disp, extra, 100, (0, 0, 255))
        _put(frame_disp, f"SERIAL:{'ON' if SERIAL_ENABLED else 'OFF'}", 115, (255, 255, 0))
