"""GPU 版巡线检测 — line_detector_v1_warp 的预处理链 CUDA 化。

与 CPU 版差异（其余逐行同源）：
  - process() 中 warpPerspective 与 blackhat→adaptive→threshold→
    形态学链走 backend（cv2.cuda，整链一次上传/下载）
  - black_th 自适应修正（adaptive mask 中位数 + Otsu 回退）保留链内
  - cv2.cuda 不可用时 backend 自动 CPU 回退，输出与 CPU 版一致
"""
...
class LineDetector:
    ...
    def process(self, bgr):
        ...
        bgr_bird = backend.birdseye_warp(bgr, self.M,
        ...
        # ── GPU 链：blackhat → adaptive → threshold → close5/open5/close5/open3
        # black_th 自适应修正（mask 中位数 + Otsu 回退）在链内等价实现
        gray_detect, binary_clean, black_th = backend.line_binary_chain(
            gray, th_offset=self.th_offset, bh_size=31, block=31, c=-8,
            th_min=self.th_min, th_max=self.th_max,
            otsu_fn=self._otsu_threshold)
        ...
        if roi_results:
            ...
            if len(all_ys) >= 3:
                ...
                if w_sum > 1e-9:
                    ...
                    if den > 1e-9:
                        a = num / den
                        angle_err = math.degrees(math.atan(a * self._asp))
                    else:
            ...
            # Source B: start line → ng_exit = start_line_z - 60cm
            ng_from_line = (start_line_z - 60.0) if start_line_z > 0 else 0.0
            ...
            if red_bar_detected and start_line_z > 0:
                red_expected = start_line_z - 36.0
                if abs(red_bar_z_cm - red_expected) < 15.0:
