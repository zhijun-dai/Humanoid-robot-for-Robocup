...
class LineDetector:
    def __init__(self, cam_w=320, cam_h=240):
        ...
        self._right_turn_scale = float(_cfg_get(cfg, "webots.right_turn_scale", 0.65))
        ...
        self._robust_steer_rate_limit = float(robust_cfg.get("steer_rate_limit_per_frame", 14.0))
        self._robust_decay_frames = int(robust_cfg.get("decay_frames", 8))

        # ── PID 参数 (用于临时状态保持，不影响输出值) ──
        self._kp_straight = float(_cfg_get(cfg, "pid.straight.kp", 0.70))
        self._ki_straight = float(_cfg_get(cfg, "pid.straight.ki", 0.015))
        self._kd_straight = float(_cfg_get(cfg, "pid.straight.kd", 0.12))
        self._kp_curve = float(_cfg_get(cfg, "pid.curve.kp", 1.05))
        self._ki_curve = float(_cfg_get(cfg, "pid.curve.ki", 0.008))
        self._kd_curve = float(_cfg_get(cfg, "pid.curve.kd", 0.18))
        self._i_clamp = float(_cfg_get(cfg, "pid.i_clamp", 60.0))

        # ── 转向参数 (用于非线性映射) ──
        self._steer_sat = float(_cfg_get(cfg, "steer.sat", 70.0))
        self._steer_scale = float(_cfg_get(cfg, "steer.scale", 22.0))
        self._steer_deadband = float(_cfg_get(cfg, "steer.deadband", 6.0))

        # ── 丢失/搜索参数 ──
        self._lost_hold_frames = int(_cfg_get(cfg, "lost.hold_frames", 6))
        self._lost_search_turn = float(_cfg_get(cfg, "lost.search_turn", 26.0))
        self._lost_prefer_left = bool(_cfg_get(cfg, "lost.prefer_left", True))
    ...
    def _nonlinear_map(v):
        steer_sat = 45.0  # 使用默认值，实际 steer 计算不影响输出
        steer_scale = 0.6
    ...
    def process(self, bgr):
        ...
        if startup_active and self._startup_force_simple_bottom:
            ...
            # 回退：逐 ROI 扫描
            roi_defs = self._build_rois()
            for roi in roi_defs:
                res = self._roi_midline(
                    gray, bgr_small, roi, black_th, track_is_dark,
                    scan_hint_center, scan_hint_width,
                )
                if res is not None and res["conf"] >= conf_min_dyn:
                    roi_results.append(res)
        ...
        if roi_results:
            ...
            # 计算 curve_mode 和 turn_gate（供调试，不影响输出）
            turn_gate = clamp(abs(curve_px) / max(self._curve_switch_px, 1.0), 0.0, 1.0)
            curve_mode_force = abs(curve_px) >= self._curve_switch_px
            if self._single_band_mask(band_mask):
                curve_mode_force = curve_mode_force or (abs(angle_err) >= 8.0)
            if (
                not bottom_lock_valid
                and bottom_pair_ratio > 0.0
                and abs(bottom_sym_err_px) > self._bottom_lock_sym_tol_px
            ):
                curve_mode_force = True

            # 运行 PID 和转向计算（用于状态一致性，不影响输出值）
            _dt = 0.1  # 默认 100ms 步长
            pid_out = self._pid_step(state["smoothed_err"], _dt, state, curve_mode_force)
            steer = self._nonlinear_map(pid_out)
            if steer < 0.0:
                steer *= self._right_turn_scale

            # 抗抖动：速率限制
            if self._robust_enable and self._robust_steer_rate_limit > 0.0:
                prev_steer = float(state["last_steer"])
                d_steer = steer - prev_steer
                if abs(d_steer) > self._robust_steer_rate_limit:
                    steer = prev_steer + math.copysign(
                        self._robust_steer_rate_limit, d_steer
                    )
    ...
    def _pid_step(self, err, dt, state, curve_mode=False, kd_scale=1.0):
        if not curve_mode:
            kp, ki, kd = self._kp_straight, self._ki_straight, self._kd_straight
        else:
            kp, ki, kd = self._kp_curve, self._ki_curve, self._kd_curve

        kd = kd * float(kd_scale)
        state["integral"] += err * dt
        state["integral"] = clamp(state["integral"], -self._i_clamp, self._i_clamp)
    ...
    def _build_rois(self):
        ...
        rois = []
        for rx, ry, rw, rh, wt in roi_ratios:
            x = int(rx * self._base_w)
            y = int(ry * self._base_h)
            w = int(rw * self._base_w)
            h = int(rh * self._base_h)
            x = max(0, x)
            y = max(0, y)
            if x + w > self._base_w:
                w = self._base_w - x
    ...
    def _roi_midline(self, gray, bgr_small, roi, black_th, track_is_dark,
                     hint_x, lane_width_hint):
        x, y, w, h, weight = roi
        x0 = x
        x1 = x + w - 1
        ...
        step = h // (self._scan_lines_per_roi + 1)
        if step < 1:
            step = 1

        last_center = hint_x
        last_width = lane_width_hint
        for i in range(1, self._scan_lines_per_roi + 1):
            sy = y + i * step
            if sy >= self._base_h:
                sy = self._base_h - 1

            lr = self._find_lr_edges_on_row(
                gray, sy, x0, x1, black_th, track_is_dark,
                last_center, last_width,
            )
            if lr is not None:
                left, right = lr
                ...
                x_cm, z_cm = self._px_to_ground_cm(center_px, sy)
                centers_cm.append(x_cm)
                centers_px.append(center_px)
                zs_cm.append(z_cm)
                lane_widths.append(lane_w)
        ...
        if len(centers_cm) < self._min_valid_lines:
            return None
        ...
        width_std = stdev(lane_widths)
        conf = (
            len(centers_cm) / float(self._scan_lines_per_roi)
        ) * (1.0 - clamp(width_std / self._width_std_max, 0.0, 1.0))
