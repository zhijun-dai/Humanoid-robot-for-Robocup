	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
		{
			...
			USART_RX_BUF[0]=Res;	
			if(Res-'0'!=state[num-1])
			{
				if (Res=='1')state[num]=1;
				if (Res=='2')state[num]=2;
				if (Res=='3')state[num]=3;
				if (Res=='4')state[num]=4;
				if (Res=='5')state[num]=5;
				if (Res=='6')state[num]=6;
				if (Res=='7')state[num]=7;
				num++;
			}
    } 
...
void uartWriteBuf3(uint8_t *buf, uint8_t len)
{
	while (len--) {
